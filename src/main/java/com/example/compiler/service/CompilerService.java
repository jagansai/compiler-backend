...
public class CompilerService {
    ...
    public CompilationResponse compile(CompilationRequest request) {
        ...
        try {
            ...
            if (!response.isSuccess()) {
                log.error("Compilation failed: {}", response.getError());
                return response; // Return early if compilation failed
            }

            // Get assembly output
            asmFile = createAssembly(request, response, sourceFile, asmFile, className);
            ...
            cleanupGeneratedFiles(sourceFile, asmFile);
        }

        return response;
    }

    private void cleanupGeneratedFiles(Path sourceFile, Path asmFile) {
        try {
            if (sourceFile != null) {
    ...
    private Path createAssembly(CompilationRequest request, CompilationResponse response, Path sourceFile, Path asmFile,
    ...
    private CompilationResponse compileCode(CompilationRequest request, CompilationResponse response, Path sourceFile)
        ...
        if (compileProcess.exitValue() != 0) {
            ...
            response.setSuccess(true);
            log.debug("Compilation succeeded for source file: {}", sourceFile);
            return response;
        }
    }
